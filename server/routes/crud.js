...
router.put("/me", verifyToken, async (req, res) => {
  try {
    ...
    const isValidOperation = updateKeys.every((key) =>
      allowedUpdates[role].includes(key)
    );
    ...
    if (role === "teacher") {
      userProfile = await Teacher.findOne({ username }).populate({
        path: "assignedClass",
        populate: { path: "students" },
      });
    } else if (role === "student") {
...
router.post("/classes", verifyToken, verifyAdmin, async (req, res) => {
  try {
    const { name, year, fees, teacher, students, studentLimit } = req.body;
    ...
    if (existingClass) {
      return res
        .status(400)
        .json({ message: "Class already exists for this year." });
    }

    const newClass = new Class({ name, year, fees, studentLimit });
...
router.put("/classes/:id", verifyToken, verifyAdmin, async (req, res) => {
  try {
    ...
    const oldTeacherId = classData.teacher
    ...
    if (newStudentsArray !== undefined) {
      const newStudentsSet = new Set(
        newStudentsArray.map((id) => id.toString())
      );
...
router.delete("/classes/:id", verifyToken, verifyAdmin, async (req, res) => {
  try {
    ...
    if (deletedClass.teacher) {
      await Teacher.findByIdAndUpdate(deletedClass.teacher, {
        $unset: { assignedClass: "" },
      });
    }
...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      await Teacher.findByIdAndUpdate(teacherObjectId, {
        assignedClass: updatedClass._id,
      });
      ...
      res
        .status(400)
        .json({ message: "Error assigning teacher", error: error.message });
    }
  }
);
...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // Avoid duplicates
      const currentStudentIds = updatedClass.students.map((id) =>
        id.toString()
      );
      ...
      res
        .status(400)
        .json({ message: "Error assigning students", error: error.message });
    }
  }
);
...
router.post("/student", verifyToken, verifyAdmin, async (req, res) => {
  try {
    ...
    if (classData.students.length >= classData.studentLimit) {
      return res
        .status(400)
        .json({ message: "Class has reached its student limit." });
    }
  ...
  try {
    ...
    if (newClassId) {
      ...
      if (!classData) {
        return res
          .status(400)
          .json({ message: "Invalid new class ID provided." });
      }
...
// GET /api/teacher/:id - (teachers can access their own profile)
router.get("/teacher/:id", verifyToken, async (req, res) => {
  try {
    ...
    // ensure they are accessing their own profile
  ...
  try {
    ...
    if (assignedClass) {
      ...
      if (!classData) {
        return res
          .status(400)
          .json({ message: "Invalid assigned class ID provided." });
      }
    ...
    const username = await generateTeacherUsername(teacher);
...
router.delete("/teacher/:id", verifyToken, verifyAdmin, async (req, res) => {
  try {
    ...
    if (deletedTeacher.assignedClass) {
      ...
      await Class.findByIdAndUpdate(deletedTeacher.assignedClass, {
        $unset: { teacher: "" },
      });
    }
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      // Combine and duplicate years
      const yearsSet = new Set(
        [...studentYears, ...teacherYears].map((y) => y.year)
      );
  ...
  async (req, res) => {
    try {
      ...
      const monthsSet = new Set(
        [...studentMonths, ...teacherMonths].map((m) => m.month)
      );
  ...
  async (req, res) => {
    try {
      ...
      if (view === "monthly" && month) {
        ...
        return res
          .status(400)
          .json({ message: "Invalid view or missing month" });
      }
      ...
      const expenses = teachers.reduce(
