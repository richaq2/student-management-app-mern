  ...
  const handleSubmit = async (e) => {
    ...
    columns.forEach((col) => {
      ...
      if (col === "feesPaidDate" && formData["feesPaid"]) {
        if (!value || value === "") {
          newErrors[col] =
            "Fees Paid Date is required when Fees Paid is checked.";
        } else {
    ...
    if (formData.hasOwnProperty("feesPaid") && !formData["feesPaid"]) {
    ...
    if (
      ...
      dataToSubmit.hasOwnProperty("class")
    ) {
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
      <div className="bg-white p-6 rounded shadow-lg w-1/2">
        ...
        <form onSubmit={handleSubmit}>
          {columns
            .filter(
              (col) => !(model === "classes" && col === "existingStudents")
            )
              ...
              <div key={col} className="mb-4">
                ...
                {col === "class" ? (
                  <>
                    ...
                    {errors[col] && (
                      <p className="text-red-500 text-sm mt-1">{errors[col]}</p>
                    )}
                  </>
                ) : col === "gender" ? (
                  <>
                  ...
                  <>
                    <input
                      ...
                      name={col}
                      value={
                        formData[col] ? formData[col].substring(0, 10) : ""
                      }
                      onChange={handleChange}
                      className={`w-full border px-3 py-2 rounded ${
                        errors[col] ? "border-red-500" : ""
